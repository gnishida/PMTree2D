...
MainWindow::MainWindow(QWidget *parent, Qt::WFlags flags) : QMainWindow(parent, flags) {
	...
	connect(ui.actionInversePMByLinearRegression4, SIGNAL(triggered()), this, SLOT(onInversePMByLinearRegression4()));
 ...
 * inverseマッピングをlinear regressionにより求める。
 * high-level indicatorとして、以下を使用する。
 *    height, width, coverage ratio, 密度のhistogram
 */
void MainWindow::onInversePMByLinearRegression2() {
	...
	for (int iter = 0; iter < N; ++iter) {
		...
		dataY(iter, 10) = glWidget->tree->stats.density_histogram[8];
		dataY(iter, 11) = glWidget->tree->stats.density_histogram[9];
		dataY(iter, 12) = 1; // 定数項
	}
 ...
 * inverseマッピングをlinear regressionにより求める。
 * high-level indicatorとして、以下を使用する。
 *    height, width, coverage ratio, 密度のhistogram、average curvature
 */
void MainWindow::onInversePMByLinearRegression3() {
	...
	for (int iter = 0; iter < N; ++iter) {
		...
		dataY(iter, 10) = glWidget->tree->stats.density_histogram[8];
		dataY(iter, 11) = glWidget->tree->stats.density_histogram[9];
		dataY(iter, 12) = glWidget->tree->stats.avg_curvature;
		dataY(iter, 13) = 1; // 定数項
	}
 ...
 * inverseマッピングをlinear regressionにより求める。
 * high-level indicatorとして、以下を使用する。
 *    height, width, coverage ratio, 密度のhistogram、curvatureのhistogram
 */
void MainWindow::onInversePMByLinearRegression4() {
	const int N = 1000;
	...
	for (int iter = 0; iter < N; ++iter) {
		...
		dataY(iter, 10) = glWidget->tree->stats.density_histogram[8];
		dataY(iter, 11) = glWidget->tree->stats.density_histogram[9];
		dataY(iter, 12) = glWidget->tree->stats.curvature_histogram[0] * lambda;
		dataY(iter, 13) = glWidget->tree->stats.curvature_histogram[1] * lambda;
		dataY(iter, 14) = glWidget->tree->stats.curvature_histogram[2] * lambda;
		dataY(iter, 15) = glWidget->tree->stats.curvature_histogram[3] * lambda;
		dataY(iter, 16) = glWidget->tree->stats.curvature_histogram[4] * lambda;
		dataY(iter, 17) = glWidget->tree->stats.curvature_histogram[5] * lambda;
		dataY(iter, 18) = glWidget->tree->stats.curvature_histogram[6] * lambda;
		dataY(iter, 19) = glWidget->tree->stats.curvature_histogram[7] * lambda;
		dataY(iter, 20) = glWidget->tree->stats.curvature_histogram[8] * lambda;
		dataY(iter, 21) = glWidget->tree->stats.curvature_histogram[9] * lambda;
		dataY(iter, 22) = 1; // 定数項
	}
	...
	for (int iter = 0; iter < 10; ++iter) {
		...
		cout << "Target: " << dataX.row(index) << endl;
		cout << "Predic: " << x_hat << endl;
		cout << endl;
